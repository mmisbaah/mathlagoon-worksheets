...
test('service worker caches the complete app shell and cleans older versions',()=>{
  ...
  assert.match(sw,/worksheet-hub-v7/);assert.ok(sw.includes('/assets/browser-icon-v2.png'));assert.ok(sw.includes('/assets/fraction-foods.png'));assert.match(sw,/caches\.delete/);assert.match(sw,/request\.mode==='navigate'/);assert.match(sw,/\['script','style'\]/);
});
