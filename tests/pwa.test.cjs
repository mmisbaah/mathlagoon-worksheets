...
test('service worker caches the complete app shell and cleans older versions',()=>{
  ...
  assert.match(sw,/worksheet-hub-v8/);assert.ok(sw.includes('/assets/browser-icon-v2.png'));assert.ok(sw.includes('/assets/fraction-foods.png'));assert.match(sw,/caches\.delete/);assert.match(sw,/request\.mode==='navigate'/);assert.match(sw,/\['script','style'\]/);
});
...
test('versioned critical assets bypass older offline caches',()=>{
  ...
  for(const asset of ['assets/interface.css?v=13','assets/core.js?v=6','assets/fractions.js?v=7','assets/interface.js?v=3','assets/progression.js?v=9','assets/printing.js?v=3'])assert.ok(html.includes(asset),asset);
});
