(function(){
  ...
  const bar=(n,d,blank=false)=>{
    const label=blank?`${d} equal unshaded parts`:`${n} of ${d} equal parts shaded`;
    const parts=Array.from({length:d},(_,i)=>`<span class="${!blank&&i<n?'shaded':''}"></span>`).join('');
    return `<div class="fraction-model" aria-label="${label}">${parts}</div>`;
  };
  ...
  const configs={
    ...
    3:{intro:'Practise fraction models and same-denominator calculations',modes:[['addSame','Add — same denominator'],['subSame','Subtract — same denominator'],['equiv','Equivalent fractions'],['missing','Fill the missing number'],['compare','Compare fractions'],['line','Number line fractions'],['write','Read a shaded model']]},
  ...
  function question(level,mode,i){
    ...
    if(mode==='shade'){const sd=level===1?(i%2?4:2):simpleD[i%5],sn=1+(i%(sd-1));return {prompt:`Shade ${sn}/${sd} of the bar. How many parts should be shaded?`,answer:String(sn),visual:bar(0,sd,true)};}
    if(mode==='write')return {prompt:'Write the fraction shown.',answer:`${n}/${d}`,visual:bar(n,d),strict:true};
    if(mode==='compare'||mode==='compareDiff'){const e=mode==='compare'?d:simpleD[(i+2)%simpleD.length],b=1+((i*2+1)%(e-1)),symbol=n/d>b/e?'>':n/d<b/e?'<':'=';return {prompt:'Write <, > or =.',answer:symbol,visual:compareBars(n,d,b,e),strict:true};}
  ...
  function install(level){
    ...
    let offset=0;
    const render=()=>{const mode=section.querySelector(`#l${level}-fractionMode`).value,count=Number(section.querySelector(`#l${level}-fractionCount`).value),grid=section.querySelector('.fraction-grid');grid.innerHTML='';for(let i=0;i<count;i++){const q=question(level,mode,(offset+i)%20),row=document.createElement('div');row.className='problem fraction-problem';row.innerHTML=`${q.visual||''}<span class="fraction-prompt">${i+1}. ${q.prompt}</span><input class="ans" inputmode="${level===1||['equiv','missing','quantity'].includes(mode)?'numeric':'text'}" autocomplete="off" aria-label="Answer to fraction question ${i+1}" data-answer="${q.answer}" data-strict="${q.strict?'true':'false'}">`;grid.append(row);}};
