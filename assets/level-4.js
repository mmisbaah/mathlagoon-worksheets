...
(function(){
  ...
  function genMathProblems(op, count) {
    const challenge = Number(document.getElementById('l4-section-math').dataset.challenge);
    mathProblems = challenge ? WorksheetCore.getChallenge(4, op, challenge) : WorksheetCore.generate(4, op, count);
    mathChecked = false;
  ...
  function renderMath() {
    ...
    mathProblems.forEach((p, i) => {
      ...
      if (p.text) {
        ...
        div.innerHTML = `${i + 1}. ${p.text || `${p.a} ${p.symbol} ${p.b} =`} <input class="ans" type="text" inputmode="decimal" data-index="${i}" autocomplete="off">`;
      }
